...
// --- Fixed held-out test observations for prediction checks ---
// Three rows not present in the training set; must match x_test / off_test in linear.R.
// These values were generated by Claude, and are hard-coded in the linear.R generation script.
...
/// Offset for test observations: 0.3 * x1 = [0.15, -0.30, 0.60].
...
/// `wald_test_against` and `score_test_against` with a non-null alternative.
/// At the MLE, both statistics must be zero (Wald because the parameter difference is zero;
/// Score because the gradient is zero). This verifies the `_against` code paths
/// beyond the null-model path already exercised by `wald_test()` / `score_test()`.
#[test]
